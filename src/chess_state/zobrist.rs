...
const fn z_piece(piece: Piece, square: u8) -> u64 {
    ...
    offset_piece += match piece.c {
        PieceColor::White => 1,
        PieceColor::Black => 0,
    };

    RANDOM64[RANDOM_PIECE_OFFSET + 64 * offset_piece + square as usize]
}

const fn z_castle_queen(color: PieceColor) -> u64 {
    match color {
        PieceColor::White => RANDOM64[RANDOM_CASTLE_OFFSET + 1],
        PieceColor::Black => RANDOM64[RANDOM_CASTLE_OFFSET + 3],
    }
}

const fn z_castle_king(color: PieceColor) -> u64 {
    match color {
        PieceColor::White => RANDOM64[RANDOM_CASTLE_OFFSET + 0],
        PieceColor::Black => RANDOM64[RANDOM_CASTLE_OFFSET + 2],
    }
}

const fn z_en_passant(square: u8) -> u64 {
    RANDOM64[RANDOM_EN_PASSANT_OFFSET + (square % 8) as usize]
}

const fn z_turn(color: PieceColor) -> u64 {
    match color {
        PieceColor::White => RANDOM64[RANDOM_TURN_OFFSET + 0],
        PieceColor::Black => 0,
    }
}
...
pub struct Zobrist;
impl Zobrist {
    pub fn calc_hash(state: &ChessState) -> u64 {
        let mut hash = 0;

        hash ^= z_turn(state.turn);

        for (square, piece) in state.pieces.iter().enumerate() {
            if let Some(p) = piece {
                hash ^= z_piece(*p, square as u8);
            }
        }
